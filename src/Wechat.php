...
class Wechat
{
     ...
     * @var string
     */
    private $mediaURL = 'https://file.api.weixin.qq.com/cgi-bin';
    ...
    /**
     * 获取跳转的URL
     * @param string $redirect_uri 跳转URI
     * @param string|null $state 状态码
     * @param string $scope
     * @return string
     */
    public function getRequestCodeURL(string $redirect_uri, string $state = null, string $scope = 'snsapi_userinfo'): string
    {
        $query = array(
            'appid' => $this->appId,
    ...
    /**
     * 获取access_token，用于后续接口访问，加入了缓存处理
     * @return string access_token信息
     */
    public function getAccessToken(string $type = 'client', $code = null): string
    {
        $cacheName = "WeinXinAccessToken_{$this->appId}_{$this->appSecret}_{$type}_{$code}";
        $data = Cache::get($cacheName);
        if (empty($data)) {
            $accessToken = $this->getNewAccessToken($type, $code);
            $data = $accessToken['access_token'];
            Cache::tag('WeinXinAccessToken')->set($cacheName, $data, $accessToken['expires_in'] - 100);
        }
        return $data;
    }
     ...
     */
    public function getNewAccessToken(string $type = 'client', $code = null): array
    {
        $param = array(
            'appid' => $this->appId,
        ...
        $token = CommonUtil::http($url, $param);
        if (is_array($token)) {
    ...
    /**
     * 获取js_ticket，用于后续接口访问
     * @return string js_ticket信息
     */
    public function getJsTicket()
    {
        $cacheName = "WeinXinJsTicket_{$this->appId}_{$this->appSecret}";
        $data = Cache::get($cacheName);
        if (empty($data)) {
            $ticket = $this->getNewJsTicket();
            $data = $ticket['ticket'];
            Cache::tag('WeinXinJsTicket')->set($cacheName, $data, $ticket['expires_in'] - 100);
        }
        return $data;
    }
    ...
    /**
     * 获取js_ticket，用于后续接口访问
     * @return array js_ticket信息，包含 ticket 和有效期
     */
    public function getNewJsTicket()
    {
        $param = array(
            'access_token' => $this->getAccessToken(),
            'type' => 'jsapi'
        );
        return CommonUtil::http("{$this->apiURL}/ticket/getticket", $param);
    }
    ...
    public function getJsConfig()
    {
        $ret = array(
            ...
            'timestamp' => time(),
            'url' => CommonUtil::getUrl(),
        );
        $ret['signature'] = $this->generateSignature($ret);
    ...
    {
        $cacheName = "ipList_{$this->appId}_{$this->appSecret}";
        $data = Cache::get($cacheName);
        if (empty($data)) {
            $data = $this->getNewIpList();
            Cache::tag('IpList')->set($cacheName, $data);
        }
        return $data;
    }
     ...
     * @throws \Exception
     */
    public function getNewIpList()
    {
        $param = array(
            'access_token' => $this->getAccessToken(),
        );
        $list = CommonUtil::http("{$this->apiURL}/getcallbackip", $param);
        return $list['ip_list'];
    }
    ...
    public function getUserInfo($token, $lang = 'zh_CN')
    {
        ...
        return CommonUtil::http("{$this->oauthApiURL}/userinfo", $query);
    }
    ...
    public function mediaUpload($filename, $type)
    {
        ...
        return CommonUtil::http($url, $param, $file, 'POST');
    }
     ...
     * @return array
     */
    public function sendTemplate($tempalteId, $toUser, $data, $url = '', $topColor = '#7B68EE')
    {
        $template = array(
            'touser' => $toUser,
        ...
        return $this->api('message/template/send', $template);
    }
    ...
    protected function api($name, $data = '', $method = 'POST', $param = '')
    {
        ...
        return CommonUtil::http($url, $params, $data, $method);
    }
